...
#include "services/agent/AgentLinkManager.hpp"
...
PetWindowController::PetWindowController(Infrastructure::ConfigManager* config, QObject* parent)
    ...
    // 多 Agent 状态感知管理器（Python: self.agent_link_manager = AgentLinkManager(self, config)）
    m_agentLink = new Services::AgentLinkManager(this, m_config, this);
}

PetWindowController::~PetWindowController() {
    if (m_agentLink != nullptr) {
        m_agentLink->stop();
    }
    savePosition();
}
...
void PetWindowController::triggerClickInteraction() {
    ...
    if (m_config && m_config->value(QStringLiteral("click_show_self_talk"), false).toBool()
        ...
        if (!bound.isEmpty()) {
            showSpeech(bound.at(QRandomGenerator::global()->bounded(bound.size())), 3000);
        } else {
            showSpeech(randomSelfTalkText(), 3000);
        }
    }
}
...
void PetWindowController::setPetVisible(bool visible, bool notify) {
    ...
    if (!visible) {
        // Python _hide_activity：隐藏时取消动作等待间歇
        cancelAnimationGap();
    }
    if (m_agentLink != nullptr) {
        // Python _hide_activity / _resume_activity：隐藏时 pause，显示时 resume
        if (visible) {
            m_agentLink->resume();
        } else {
            m_agentLink->pause();
...
void PetWindowController::onFullscreenCheck() {
    bool isFs = m_config && m_config->autoHideFullscreen()
...
void PetWindowController::resetToBottomRight() {
    ...
    const double targetX = avail.right() - w - 40.0;
    const double targetY = avail.bottom() - h - 40.0;
...
void PetWindowController::toggleAgentLink(const QString& agentKey, bool enabled, QAction* action) {
    // Python _toggle_agent_link：set_enabled 返回 False（hooks 安装失败 / 未知 key）时，
    // 必须把菜单勾选态回滚，否则 UI 显示已开启而实际未生效。
    if (m_config == nullptr || agentKey.isEmpty()) {
        return;
    }
    if (m_agentLink != nullptr) {
        const bool ok = m_agentLink->setEnabled(agentKey, enabled);
        if (!ok) {
            if (action != nullptr) {
