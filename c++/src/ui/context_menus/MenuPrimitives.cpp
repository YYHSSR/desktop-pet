...
QMenu* addAgentLinkMenu(QMenu* menu, const MenuContext& context) {
    ...
    for (const Core::AgentMenuItem& item : Core::agentMenuItems()) {
        QAction* action = submenu->addAction(item.label);
        action->setCheckable(true);
        action->setChecked(values.value(item.key).toBool());
        QObject::connect(action, &QAction::toggled, submenu,
                         [context, key = item.key, action](bool on) {
            if (context.controller != nullptr) {
                // 失败/被拒时由 controller 回滚勾选态。
