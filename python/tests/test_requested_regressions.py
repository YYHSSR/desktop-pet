...
def test_macos_hide_pet_enables_dock_icon(tmp_path, monkeypatch):
    ...
    from pet.infrastructure import desktop_platform
    ...
    monkeypatch.setattr(desktop_platform, "_mac_set_dock_icon_visible", mock.Mock())
    ...
    desktop_platform._mac_set_dock_icon_visible.assert_called_once_with(True)



    # 恢复显示：按偏好还原 Dock 策略（pref=False → Accessory）

    desktop_platform._mac_set_dock_icon_visible.reset_mock()

    PetWindow._restore_dock_icon_preference(fake)

    assert getattr(fake, "_dock_icon_forced", True) is False

    desktop_platform._mac_set_dock_icon_visible.assert_called_once_with(False)
