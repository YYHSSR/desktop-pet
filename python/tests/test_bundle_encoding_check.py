...
def test_windows_build_collects_conda_sqlite_and_ffi_runtime_dlls():
    script = (Path(__file__).parents[1] / "scripts" / "build_onedir.ps1").read_text(encoding="utf-8")
    assert "Library\\bin\\sqlite3.dll" in script
    assert "Library\\bin\\ffi.dll" in script
    assert "@runtimeDlls" in script


def test_builds_do_not_bundle_removed_dsh_bridge():
    root = Path(__file__).parents[1]
    paths = [
        root / "scripts" / "build_onedir.ps1",
    ...
    for path in paths:
        ...
        assert "integrations/dsh-pet-bridge" not in text
        assert "integrations;integrations" not in text
        assert '"integrations:integrations"' not in text
