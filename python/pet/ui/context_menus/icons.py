...
def vector_menu_icon(menu: QMenu, name: str, size: int | None = None) -> QIcon:
    ...
    elif name in {"agent", "agent_link", "robot"}:
        painter.drawLine(QPointF(8.0, 1.8), QPointF(8.0, 3.8))
        painter.drawEllipse(QPointF(8.0, 1.4), 0.7, 0.7)
        painter.drawRoundedRect(QRectF(2.8, 3.8, 10.4, 8.4), 2.0, 2.0)
        painter.drawLine(QPointF(1.4, 7.2), QPointF(2.8, 7.2))
        painter.drawLine(QPointF(13.2, 7.2), QPointF(14.6, 7.2))
        painter.setBrush(QBrush(color))
        painter.drawEllipse(QPointF(5.8, 7.5), 0.9, 0.9)
        painter.drawEllipse(QPointF(10.2, 7.5), 0.9, 0.9)
        painter.setBrush(Qt.BrushStyle.NoBrush)
        painter.drawLine(QPointF(5.6, 10.0), QPointF(10.4, 10.0))
    elif name == "hide":
