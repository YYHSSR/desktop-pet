...
def characters_dir() -> Path:
    ...
    return resource_root() / 'assets' / 'characters'
...
def characters_gif_dir() -> Path:
    ...
    return resource_root() / 'assets' / 'characters_gif'
...
def external_character_dirs() -> list[Path]:
    ...
    2. 打包变体数据目录下的 characters/（变体感知，如 desktop-pet-webm-chat）
    3. 通用目录 desktop-pet/characters/ 兜底

    数据目录名复用 config.APP_DIR_NAME。配置层只依赖 core.defaults，
    不再反向依赖角色目录；此处按调用时读取，支持运行期变体覆盖。
    """
    dirs: list[Path] = []
    ...
    from pet.infrastructure import config as _config
    app_dir_name = getattr(_config, 'APP_DIR_NAME', 'desktop-pet')
    dirs.extend(_data_character_dirs(app_dir_name))
    # 非变体旧目录兜底：老用户已放入的角色不能因为变体拆分而丢
    if app_dir_name != 'desktop-pet':
        dirs.extend(_data_character_dirs('desktop-pet'))
    return dirs
