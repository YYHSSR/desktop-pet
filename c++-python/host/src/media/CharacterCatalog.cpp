...
void CharacterCatalog::scanExternalCharacters(const QString& charactersRoot) {
    QDir rootDir(charactersRoot);
    if (!rootDir.exists()) {
        return;
    }
    ...
    for (const QString& entry : entries) {
        if (!m_characters.contains(entry)) {
            m_characters.append(entry);
            changed = true;
        }
        // 同名角色以内置为准：仅当尚未注册时记录外部根目录
        if (!m_charRoots.contains(entry) && !QDir(rootDir.filePath(entry)).isEmpty()) {
            m_charRoots.insert(entry, rootDir.absolutePath());
            changed = true;
        }
    }
    if (changed) {
...
QString CharacterCatalog::characterRootFor(const QString& character) const {
    // 外部注册的角色解析到其所在根目录，其余走内置 assets 根
    return m_charRoots.value(character, m_charactersRoot);
}
...
QString CharacterCatalog::resolveAnimationPath(const QString& character, PetActionState action) const {
    ...
    QDir baseDir(QDir(characterRootFor(character)).filePath(charName));
    if (!baseDir.exists() && !m_characters.isEmpty()) {
        charName = m_characters.first();
        baseDir = QDir(QDir(characterRootFor(character)).filePath(charName));
    }
    if (!baseDir.exists()) {
        return "";
    }
...
QString CharacterCatalog::resolveAnimationPath(const QString& character, const QString& animName) const {
    QDir baseDir(QDir(characterRootFor(character)).filePath(character));
    if (!baseDir.exists()) {
        return "";
    }
...
QMap<QString, QStringList> CharacterCatalog::getAnimationCategories(const QString& character) const {
    ...
    QDir charDir(QDir(characterRootFor(character)).filePath(character));
    if (!charDir.exists()) {
        return result;
    }
...
QStringList CharacterCatalog::animationNames(const QString& character, const QString& folder) const {
    ...
    QStringList names;
    const QDir charDir(QDir(characterRootFor(character)).filePath(character));
